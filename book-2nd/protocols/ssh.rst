...
:term:`telnet` runs over TCP and a telnet server uses port 23 by default.
The TCP connection used by telnet is bidirectional, both the client and
the server can send data over it. The data exchanged over such a connection
is essentially the characters that are typed by the user on the
client machine and the text output of the process running on the server
machine with a few exceptions. The default character set for
telnet is the ASCII character set, but the extensions specified
in :rfc:`5198` support the utilisation
of Unicode characters.

From a security viewpoint, the main drawback of telnet is that all
the information, including the usernames, passwords and commands,
is sent in cleartext over the TCP connection. This implies that
an eavesdropper could easily capture the passwords used by anyone
on an unprotected network and there are various software tools
that automate this collection of information. For this reason,
:term:`telnet` is rarely used to access remote computers and
is usually replaced by :term:`ssh` or similar protocols.
...
develop an open-source version of :term`ssh` called OpenSSH.
...
The :term:`ssh` protocol runs above the TCP protocol. Once the bytestream
...
be considered totally secure and are recommended today and could
...
Once the algorithms have been negotiated, the key exchange algorithm is
used to negotiate a secret key that is shared by the client and the server.
These key exchange algorithms include some variations over
the basic algorithms. As an explained, let us analyse how the
Diffie Hellman key exchanged algorithm is used within the
``ssh`` protocol. Each host has both a private and a public key. 
 ...
 - the client can recompute :math:`K=A^{b} mod p` and then verify the 
   signature provided by the server

This is a slightly modified authenticated Diffie Hellman key exchange
with two interesting points that must be noted. The first point is that 
...
can decide to accept or reject the key. This warning can me associated
...
of the Diffie Hellman exchange but a hash of all the information
sent and received during this exchange. This is important to prevent
...
encryption scheme that will be easier to break by the attacker. Thanks
...
.. note:: Single use keys
   ...
   directly used this key for all the cryptographic algorithms that
   have been negotiated for this session. Like most security protocols,
   ``ssh`` does not directly uses key :math:`K`. Instead, it uses
...
At this point, the client and server only exchange messages that are
encrypted with the negotiated keys. The ``ssh`` protocol uses messages
...
:rfc:`4253`. Each of these messages contains the following information :
 ...
 - ``payload`` : the data (after compression) passed by the user
...
commande on ``computer2``, she can create an ``ssh`` session on this computer
...
.. [#fsshkeys] The exact algorithms used for the computation of these
               keys are defined in :rfc:`4251` CHECK!
